...
async def cancel_notification(trip: TripRead):
...
async def check_need_to_create_task_immediately(trip: TripRead):
    notification_time = (trip.travel_date -
                         (trip.notification_before_travel - dt.datetime.fromisoformat('1970-01-01')))
    last_check_time = await get_last_check_notification_time()
